...
class FloatingLabel extends Component {
  ...
  render() {
    const labelColor = this.state.color.interpolate({
      inputRange: [0, 1],
...
class Textfield extends Component {
  ...
  set bufferedValue(v) {
    ...
    if (v) {
      ...
      if (!this.isFocused()) { this._aniStopHighlight(); }
    }
  }
  ...
  componentWillMount() {
    this.bufferedValue = this.props.value;
    this._originPlaceholder = this.props.placeholder;
  }

  componentWillReceiveProps(nextProps) {
    if (nextProps.value === '' && this.bufferedValue !== '') {
      this.bufferedValue = '';
    }
  ...
  _aniStopHighlight() {
    ...
    if (this.props.floatingLabelEnabled) {
      if (!this.bufferedValue) {
        ...
        animations.push(...this.refs.floatingLabel.aniStopHighlightLabel());
      }
    }
