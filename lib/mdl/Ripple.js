...
class Ripple extends Component {
  constructor(props) {
    ...
    if (props.style && props.style.length) {
      props.style[0] && props.style[0].forEach && props.style[0].forEach(item => {
        if (item.shadowOffset) {
          this.shadowOffsetHeight = item.shadowOffset.height;
        }
      });
    }
    ...
    this.state = {
      ...
      height: 1,
      maskBorderRadius: 0,
      shadowOffsetY: this.shadowOffsetHeight,
  ...
  showRipple() {
    ...
    if (this.props.shadowAniEnabled) {
      this.setState({ shadowOffsetY: this.shadowOffsetHeight * 1.5 });
    }
  ...
  hideRipple() {
    this._pendingRippleAni = () => {
      ...
      if (this.props.shadowAniEnabled) {
        this.setState({ shadowOffsetY: this.shadowOffsetHeight });
      }
